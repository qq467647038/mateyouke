...
class Goods extends Common{
    ...
    public function catelist(){
        if(input('cate_id')){
            ...
            $cates = Db::name('shop_cate')->where('id',$cid)->where('shop_id',$shop_id)->find();
            if($cates){
                $filter = input('filter');
                if(!$filter || !in_array($filter, array(1,2,3))){
                    $filter = 3;
    ...
    public function uploadifyattr(){
        ...
        if($file){
            ...
            if($info){
                ...
                $image = \think\Image::open('./'.$original);
                $image->thumb(640, 400)->save('./'.$original,null,90);
                $zspic_id = Db::name('shopadmin_zsattrpic')->insertGetId(array('admin_id'=>$admin_id,'img_url'=>$original));
                $picarr = array('img_url'=>$original,'pic_id'=>$zspic_id);
                $value = array('status'=>1,'path'=>$picarr);
            }else{
                $value = array('status'=>0,'msg'=>$file->getError());
    ...
    public function delattrfile(){
        if(input('post.zspic_id')){
            $admin_id = session('shopadmin_id');
            $zspic_id = input('post.zspic_id');
            $pics = Db::name('shopadmin_zsattrpic')->where('id',$zspic_id)->where('admin_id',$admin_id)->find();
            if($pics && $pics['img_url']){
    ...
    public function uploadifys(){
        ...
        $nupload = Db::name('shopadmin_zsduopic')->where('admin_id',$admin_id)->count();
        ...
        if($file){
            ...
            if($info){
                ...
                $image = \think\Image::open('./'.$original);
                $image->thumb(640, 400)->save('./'.$original,null,90);
                $pic_id = Db::name('shopadmin_zsduopic')->insertGetId(array('img_url'=>$original,'admin_id'=>$admin_id));
    ...
    public function deletefile(){
        if(input('post.pic_id')){
            ...
            $admin_id = session('shopadmin_id');
            $img_url = Db::name('shopadmin_zsduopic')->where('id',$pic_id)->where('admin_id',$admin_id)->value('img_url');
            if($img_url){
    ...
    public function deleteone(){
        ...
        if(input('post.ypic_id') && input('post.goods_id')){
            $goods = Db::name('goods')->where('id',input('post.goods_id'))->where('shop_id',$shop_id)->where('is_recycle',0)->field('id')->find();
            if($goods){
        ...
        if(request()->isPost()){
            ...
            if(true !== $result){
                ...
                if($categorys){
                    ...
                    if(!$child_cates){
                        $cateidres = Db::name('manage_cate')->where('shop_id',$shop_id)->where('checked',1)->field('cate_id')->select();
                        $catearr = array();
                        ...
                        if($categorys['pid'] == 0){
                            ...
                            $cateIds = array();
                            $cateIds = get_all_parent($categoryres, $data['cate_id']);
                            $cid = end($cateIds);
                        }
                        ...
                        if(in_array($cid, $catearr)){
                            ...
                            if($shcates){
                                ...
                                if(!$child_shcates){
                                        ...
                                        if(!empty($data['pic_id'])){
                                            ...
                                            if($zssjpics && $zssjpics['img_url']){
                                                            ...
                                                            if($yzshuxing){
                                                                if($yzshuxing['attr_type'] == 1){
                                                                    if(!empty($yzval['attr_value']) && is_array($yzval['attr_value'])){
                                                                        ...
                                                                        $value = array('status'=>0,'mess'=>'存在属性值为空或参数错误，增加失败');
                                                                        return json($value);
                                                                    }
                                                ...
                                                try{
                                                    ...
                                                    if($goods_id){
                                                        ...
                                                        if(!empty($data['picres_id'])){
                                                            ...
                                                            foreach ($data['picres_id'] as $key => $val){
                                                                $img_url = Db::name('shopadmin_zsduopic')->where('id',$val)->where('admin_id',$admin_id)->value('img_url');
                                                                if($img_url){
                                                                    if(empty($sort2[$key])){
                                                                        $sort2[$key] = 0;
                                                                    }
                                                                        ...
                                                                        for($i=0; $i<count($val2['attr_value']); $i++){
                                                                            if(!empty(trim($val2['attr_value'][$i]))){
                                                                                if($attrshuxing['is_upload'] == 1 && !empty($val2['attrpic_id'][$i])){
                                                                                    $attr_pic = Db::name('shopadmin_zsattrpic')->where('id',$val2['attrpic_id'][$i])->value('img_url');
                                                                                }else{
                                                                                    $attr_pic = '';
                                                                                }
                                                    ...
                                                    $zsattrpics = Db::name('shopadmin_zsattrpic')->where('admin_id',$admin_id)->field('id,img_url')->select();
                                                    if($zsattrpics){
                                                        foreach ($zsattrpics as $v){
                                                            Db::name('shopadmin_zsattrpic')->delete($v['id']);
                                                        }
                                                    }
                                    
                                                    if(!empty($data['picres_id'])){
                                                        $zsinduspics = Db::name('shopadmin_zsduopic')->where('admin_id',$admin_id)->field('id,img_url')->select();
                                                        if($zsinduspics){
                                                            foreach ($zsinduspics as $v){
                                                                Db::name('shopadmin_zsduopic')->delete($v['id']);
                                                            }
                                                        }
                                                    }
                                    ...
                                    $value = array('status'=>0,'mess'=>'店铺分类存在下级分类，新增失败');
                                }
                            }else{
                                $value = array('status'=>0,'mess'=>'参数错误');
                            }
                        }else{
                            $value = array('status'=>0,'mess'=>'商品分类不属于经营类目，新增失败');
                        }
                    }else{
                        ...
                        if($categorys){
                            ...
                            if(!$child_cates){
                                ...
                                if(in_array($cid, $catearr)){
                                        ...
                                        if(!$child_shcates){
                                            ...
                                            if($gdtypes && $data['type_id'] == $categorys['type_id']){
                                                ...
                                                /*if(!empty($data['mp'])){
                                                 ...
                                                 $value = array('status'=>0,'mess'=>'会员价格等级参数错误，编辑失败');
                                                 return json($value);
                                                 }
                                                 }
                                                 if($yzlevels != $levelres){
                                                 $value = array('status'=>0,'mess'=>'会员价格不完整，编辑失败');
                                                 return json($value);
                                                 }
                                                 }*/
                                                ...
                                                if($countnum <= $webconfig['goodsimg_maxnum']){
                                                    ...
                                                    try{
                                                        ...
                                                        /*if(!empty($data['mp'])){
                                                         ...
                                                         $v = trim($v);
                                                         if(!empty($mpid[$k])){
                                                         $memberprices = Db::name('member_price')->where('id',$mpid[$k])->where('goods_id',$goods_id)->find();
                                                         if($memberprices){
                                                         Db::name('member_price')->where('id',$mpid[$k])->where('goods_id',$goods_id)->update(array('price'=>$v));
                                                         }
                                                         }else{
                                                         $memberprices = Db::name('member_price')->where('level_id',$k)->where('goods_id',$goods_id)->find();
                                                         if(!$memberprices){
                                                         Db::name('member_price')->insert(array(
                                                         'price'=>$v,
                                                         'level_id'=>$k,
                                                        ...
                                                        if($goodss['type_id'] != $data['type_id']){
                                                            $gaids = Db::name('goods_attr')->alias('a')->field('a.id,a.attr_pic')->join('sp_attr b','a.attr_id = b.id','INNER')->where('a.goods_id',$goods_id)->where('b.attr_type',1)->select();
                                                            ...
                                                            if($gaids){
                                                                foreach ($gaids as $va){
                                                                    ...
                                                                    if($va['attr_pic'] && file_exists('./'.$va['attr_pic'])){
                                                                        @unlink('./'.$va['attr_pic']);
                                                                    }
                                                                }
                                                            }else{
                                    ...
                                    $value = array('status'=>0,'mess'=>'商品分类不属于经营类目，编辑失败');
                                }
                            }else{

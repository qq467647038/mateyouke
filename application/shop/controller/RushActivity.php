...
class RushActivity extends Common{
    ...
    public function lst(){
        ...
        $list = Db::name('rush_activity')->alias('a')->field('a.*,b.goods_name')->join('sp_goods b','a.goods_id = b.id','LEFT')->where($where)->order('a.apply_time desc')->paginate(25);
        $page = $list->render();
    ...
    public function add(){
        if(request()->isPost()){
            ...
            $result = $this->validate($data,'RushActivity');
            if(true !== $result){
                    ...
                    if($start_time < $end_time){
                        ...
                        if(!$group_buys){
                            ...
                            if(!$assembles){
                                ...
                                if($rush_activitys){
                                    if(!$rush_activitys['goods_attr']){
                                        $value = array('status'=>0,'mess'=>'活动时间内该商品全部规格已参与抢购，新增失败');
                                        return json($value);
                                    }else{
                                        if(input('post.goods_attr')){
                                            ...
                                            if($garr == $rush_activitys['goods_attr']){
                                                $value = array('status'=>0,'mess'=>'活动时间内商品所选规格已参与抢购，新增失败');
                                                return json($value);
                                            }elseif(input('post.goods_attr') == '*'){
                                                $value = array('status'=>0,'mess'=>'活动时间内已有商品相关规格参与抢购，请删除后重试');
                                                return json($value);
                                            }
                                        }else{
                                ...
                                if($data['price'] <= $shop_price){
                                    if($data['num'] <= $goods_number){
                                        $lastId = Db::name('rush_activity')->insert(array(
                                        ...
                                        $value = array('status'=>0,'mess'=>'商品库存不足，请重新填写抢购量');
                                    }
                                }else{
                                    $value = array('status'=>0,'mess'=>'抢购价格不得大于商品价格');
                                }
                            }else{
    ...
    public function info(){
        if(input('id')){
            ...
            $rushs = Db::name('rush_activity')->alias('a')->field('a.*,b.goods_name,b.shop_price,b.min_price,b.max_price,b.thumb_url,c.shop_name')->join('sp_goods b','a.goods_id = b.id','LEFT')->join('sp_shops c','a.shop_id = c.id','LEFT')->where('a.id',input('id'))->where('a.shop_id',$shop_id)->find();
            if($rushs){
                if($rushs['goods_attr']){
                    if($rushs['goods_attr'] != '*'){
    ...
    public function delete(){
        ...
        $id = input('id');
        if(!empty($id)){
            $rushs = Db::name('rush_activity')->where('id',$id)->where('shop_id',$shop_id)->where('is_show',1)->find();
            if($rushs){
                ...
                Db::startTrans();
                try{
                    Db::name('rush_activity')->update(array('is_show'=>0,'id'=>$id));
    ...
    public function ceshi(){
        $goodres = Db::name('goods')->field('id,shop_price')->select();
        
        foreach($goodres as $v){
            $radiores = Db::name('goods_attr')->alias('a')->field('a.id,a.attr_id,a.attr_value,a.attr_price')->join('sp_attr b','a.attr_id = b.id','INNER')->where('a.goods_id',$v['id'])->where('b.attr_type',1)->select();
            if($radiores){
                $radioattr = array();
                ...
                $min_shop_price = $v['shop_price']+$min_attr_price;
                $max_shop_price = $v['shop_price']+$max_attr_price;
            }else{
                $min_shop_price = $v['shop_price'];
                $max_shop_price = $v['shop_price'];
            }
            Db::name('goods')->update(array('min_price'=>$min_shop_price,'max_price'=>$max_shop_price,'id'=>$v['id']));
        }
    ...
    public function search(){
        ...
        if(input('post.keyword') != ''){
            cookie('rush_keyword',input('post.keyword'),7200);
        }else{
            cookie('rush_keyword',null);
        }
        
        if(input('post.checked') != ''){
            cookie('rush_checked',input('post.checked'),7200);
        }
        
        if(input('post.recommend') != ''){
            cookie('rush_recommend',input('post.recommend'),7200);
        }
        
        if(input('post.starttime') != ''){
            $rushstarttime = strtotime(input('post.starttime'));
            cookie('rushstarttime',$rushstarttime,7200);
        }
        
        if(input('post.endtime') != ''){
            $rushendtime = strtotime(input('post.endtime'));
            cookie('rushendtime',$rushendtime,7200);
        }
        ...
        if(cookie('rush_keyword')){
            $where['a.activity_name'] = cookie('rush_keyword');
        }
        
        if(cookie('rush_checked') != ''){
            $rush_checked = (int)cookie('rush_checked');
            if(!empty($rush_checked)){
                switch ($rush_checked){
                    //待审核
                    case 1:
                        $where['a.checked'] = 0;
        ...
        if(cookie('rush_recommend') != ''){
            $rush_recommend = (int)cookie('rush_recommend');
            if(!empty($rush_recommend)){
                switch ($rush_recommend){
                    //推荐
                    case 1:
                        $where['a.recommend'] = 1;
                        break;
        ...
        if(cookie('rushendtime') && cookie('rushstarttime')){
            $where['a.apply_time'] = array(array('egt',cookie('rushstarttime')), array('elt',cookie('rushendtime')));
        }
        
        if(cookie('rushstarttime') && !cookie('rushendtime')){
            $where['a.apply_time'] = array('egt',cookie('rushstarttime'));
        }
        
        if(cookie('rushendtime') && !cookie('rushstarttime')){
            $where['a.apply_time'] = array('elt',cookie('rushendtime'));
        }
        ...
        if(cookie('rushstarttime')){
            $this->assign('starttime',cookie('rushstarttime'));
        }
        
        if(cookie('rushendtime')){
            $this->assign('endtime',cookie('rushendtime'));
        }
        
        if(cookie('rush_recommend')){
            $this->assign('recommend',cookie('rush_recommend'));
        }
        
        if(cookie('rush_checked')){
            $this->assign('checked',cookie('rush_checked'));
        }
        
        if(cookie('rush_keyword')){
            $this->assign('keyword',cookie('rush_keyword'));
        }

...
class WechatUtil
{
    ...
    private function requestAndCheck($url, $method = 'GET', $fields = [])
    {
        ...
        if (isset($wxdata['errcode']) && $wxdata['errcode'] != 0) {
            if ($wxdata['errcode'] == 40001) {
                Model('wx_user')->where('id', $this->config['id'])->save(['web_expires' => 0]);//token错误
            }
            if ($this->debug) {
    ...
    public function getAccessToken()
    {
        ...
        Model('wx_user')->where('id', $wechat['id'])->save(['web_access_token'=>$return['access_token'], 'web_expires'=>$web_expires]);

...
class ComWxPay
{
    ...
    public function getPrePayOrder($body, $out_trade_no, $total_fee, $time_start, $time_expire, $notify_url, $userInfo)
    {
        ...
        // $data['openid'] = $userInfo['xcx_openid'];
        $data['openid'] = $userInfo['wx_openid'];
        ...
        $data["sign"] = $s;
        // Db::name('test')->insert(['value' => json_encode($data)]);
        $xml = $this->arrayToXml($data);
        $response = $this->postXmlCurl($xml, $url);
        // Db::name('test')->insert(['value' => $response]);
        // Db::name('test')->insert(['value' => json_encode($test)]);
        //将微信返回的结果xml转成数组

...
//                            }
    ...
    public function add()
    {
        if (request()->isPost()) {
            $data = input('post.');
//            $admin_id = session('admin_id');
            $result = $this->validate($data, 'WineGoods');
            if (true !== $result) {
            ...
            Db::startTrans();
            try {
                $goods_id = Db::name('wine_goods')->insertGetId(array(
                ...
                // 提交事务
                Db::commit();
                ys_admin_logs('添加酒业商品', 'wine_goods', $goods_id);
...
//                $data['shop_id'] = $shop_id;
                ...
                if (true !== $result) {
                    ...
                    $goodss = Db::name('wine_goods')->where('id', input('post.id'))->find();
                    if ($goodss) {
...
//                                                $data['search_keywords'] = str_replace('，', ',', $data['search_keywords']);
                                                ...
                                                try {
                                                    ...
                                                    Db::name('wine_goods')->update($update);
                                                    ...
                                                    ys_admin_logs('编辑酒业商品', 'goods', $data['id']);
...
//                $shop_id = session('shop_id');
//                $admin_id = session('admin_id');
                $goodss = Db::name('wine_goods')->where('id', input('id'))->find();
                if ($goodss) {

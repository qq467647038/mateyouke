...
class PromActivity extends Common{
    ...
    public function lst(){
        ...
        $list = Db::name('prom_activity')->where($where)->field('id,recommend,man_price,activity_name,type,discount,reduction,start_time,end_time,pic_url')->order('addtime desc')->paginate(25);
        $page = $list->render();
    ...
    public function add(){
        if(request()->isPost()){
            ...
            $result = $this->validate($data,'PromActivity');
            if(true !== $result){
    ...
    public function search(){
        ...
        $list = Db::name('prom_activity')->where($where)->field('id,activity_name,type,discount,reduction,man_price,start_time,end_time,pic_url')->order('addtime desc')->paginate(25);
        $page = $list->render();
    ...
    public function gaibian(){
        ...
        $rushs = Db::name('prom_activity')->where('id',$id)->where('shop_id','neq',$shop_id)->find();
        if($rushs){
            $data[$name] = $value;
            $data['id'] = $id;
            $count = Db::name('prom_activity')->update($data);
            if($count > 0){
                ys_admin_logs('修改商家商品促销活动推荐状态','prom_activity',$id);
                $result = 1;
            }else{
                $result = 0;
            }

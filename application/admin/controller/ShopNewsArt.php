...
use think\Db;
use app\admin\model\ShopNews as NewsMx;

class ShopNewsArt extends Common{
    //文章列表
    public function lst(){
        $p = input('p/d', 1);
        $size = input('size/d', 20);
        $list = Db::name('shop_news')->alias('a')->field('a.id,a.ar_title,a.is_rem,a.useable,a.addtime,a.sort,b.cate_name,c.en_name')->join('sp_cate_art b','a.cate_id = b.id','LEFT')->join('sp_admin c','a.aid = c.id','LEFT')->order('a.id desc')->paginate($size);
        $page = $list->render();
        $cateres = Db::name('cate_art')->field('id,cate_name,pid')->order('sort asc')->select();
        if(input('page')){
            $pnum = input('page');
        }else{
            $pnum = 1;
    ...
    public function catelist(){
        $id = input('cate_id');
        $cate_name = Db::name('cate_art')->where('id',$id)->value('cate_name');
        $cateres = Db::name('cate_art')->field('id,cate_name,pid')->order('sort asc')->select();
        $cateId = array();
        $cateId = get_all_child($cateres, $id);
        $cateId[] = $id;
        $cateId = implode(',', $cateId);
        $list = Db::name('shop_news')->alias('a')->field('a.id,a.ar_title,a.is_rem,a.useable,a.addtime,a.sort,b.cate_name,c.en_name')->join('sp_cate_art b','a.cate_id = b.id','LEFT')->join('sp_admin c','a.aid = c.id','LEFT')->where('a.cate_id','in',$cateId)->order('a.sort asc')->paginate(25);
        $page = $list->render();
        if(input('page')){
            $pnum = input('page');
        }else{
    ...
    public function uploadify(){
        ...
        if($file){
            $info = $file->validate(['size'=>3145728,'ext'=>'jpg,png,gif,jpeg'])->move(ROOT_PATH . 'public' . DS . 'uploads' . DS . 'new_pic_art');
            if($info){
                ...
                $original = 'uploads/new_pic_art/'.$getSaveName;
    ...
    public function gaibian(){
        ...
        // $data['id'] = $id;
        // $news = new NewsMx();
        $count = Db::name('shop_news')->where(['id'=>$id])->update($data);
        if($count > 0){
            $result = 1;
        }else{
            $result = 0;
    ...
    public function add(){
        if(request()->isAjax()){
            ...
            $cateres = Db::name('cate_art')->field('id,pid,cate_name')->order('sort asc')->select();
    ...
    public function edit(){
        if(request()->isAjax()){
            ...
            if(true !== $result){
                ...
                $ars = Db::name('shop_news')->where('id',$data['id'])->find();
                if($ars){
                    if(!empty($data['pic_id'])){
                        $zssjpics = Db::name('huamu_zspic')->where('id',$data['pic_id'])->where('admin_id',$admin_id)->find();
                        if($zssjpics && $zssjpics['img_url']){
                    ...
                    if(isset($data['id']) && $data['id']>0)unset($data['addtime']);
    ...
    public function delete(){
        ...
        if(!empty($id)){
            if(is_array($id)){
                $delId = implode(',', $id);
                $pic = Db::name('shop_news')->where('id','in',$delId)->field('ar_pic')->order('addtime desc')->select();
            }else{
                $pic =  Db::name('shop_news')->where('id',$id)->value('ar_pic');
            }
            $count = Db::name('shop_news')->delete($id);
            if($count > 0){
                if(is_array($id)){
                    if(!empty($pic)){
                        foreach ($pic as $v){
    ...
    public function search(){
        ...
        $list = Db::name('shop_news')->alias('a')->field('a.id,a.ar_title,a.is_rem,a.useable,a.addtime,a.sort,b.cate_name,c.en_name')->join('sp_cate_art b','a.cate_id = b.id','LEFT')->join('sp_admin c','a.aid = c.id','LEFT')->where($where)->order('a.sort asc')->paginate(25);
        $page = $list->render();
        if(input('page')){
            $pnum = input('page');
        }else{
    ...
    public function paixu(){
       if(request()->isAjax()){
            if(input('post.ids') && input('post.sort')){
                ...
                $sort = explode(',', $sort);
                foreach ($ids as $k => $v){
                    Db::name('shop_news')->update(array('id'=>$v,'sort'=>$sort[$k]));
                }
            }

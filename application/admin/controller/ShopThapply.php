...
class ShopThApply extends Common{
    ...
    public function savewuliu(){
        ...
        $order_id = input('post.order_id');
        if(empty($ps_id) || empty($psnum)){
            $value = array('status'=>1,'mess'=>'操作失败，请联系商家增加收货地址');
            return json($value);    
        }
        $wuliu_infos = Db::name('order_wuliu')->where('pt_psnum',$psnum)->find();
        if(!$wuliu_infos){
            $count = Db::name('order')->update(array('o_state'=>10,'id'=>$order_id));
    ...
    public function search(){
        ...
        if(cookie('shth_keyword')){
            $where['a.ordernumber'] = cookie('shth_keyword');
        }

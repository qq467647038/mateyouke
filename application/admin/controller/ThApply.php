...
class ThApply extends Common{
        ...
        if(request()->isPost()){
            if(input('post.th_id')){
                if(input('post.apply_status') && in_array(input('post.apply_status'), array(1,2))){
                    ...
                    $applys = Db::name('th_apply')->where('id',$th_id)->where('shop_id',$shop_id)->where('apply_status',0)->where('check_timeout','gt',time())->find();
                    if($applys){
        ...
        if(request()->isPost()){
            if(input('post.th_id')){
                ...
                if($applys){
                    ...
                    //修改订单状态为10
                    Db::name('order')->update(['o_state'=>10,'id'=>$applys['order_id']]);
                    if($count > 0){
                ...
                if($applys){
                    ...
                    if($orgoods){
                        ...
                        try{
                            ...
                            $ordergoods = Db::name('order_goods')->where('id','neq',$applys['orgoods_id'])->where('order_id',$applys['order_id'])->where('th_status','in','0,1,2,3,5,6,7,8')->field('id,th_status')->find();
                            if(!$ordergoods){
    ...
    public function search(){
        ...
        if(cookie('th_keyword')){
            $where['a.ordernumber'] = cookie('th_keyword');
        }

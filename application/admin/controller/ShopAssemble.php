...
class ShopAssemble extends Common{
    ...
    public function info(){
        if(input('id')){
            ...
            $assembles = Db::name('assemble')->alias('a')->field('a.*,b.goods_name,b.shop_price,b.min_price,b.max_price,b.thumb_url,c.shop_name')->join('sp_goods b','a.goods_id = b.id','LEFT')->join('sp_shops c','a.shop_id = c.id','LEFT')->where('a.id',input('id'))->where('a.is_show',1)->where('a.shop_id','neq',$shop_id)->find();
            if($assembles){
                if($assembles['goods_attr']){
                    if($assembles['goods_attr'] != '*'){
                        $assembles['goods_attr_str'] = '';

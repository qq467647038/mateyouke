...
use think\Db;
use app\admin\model\CateArt as CateNewMx;

class CateArt extends Common{
    //栏目列表
    public function category(){
        $p = input('p/d', 1);
        $size = input('size/d', 20);
        $list = Db::name('cate_art')->field('id,cate_name,pid,sort,useable,show_in_recommend')->order('sort asc')->paginate($size);
    ...
    public function gaibian(){
        ...
        if($count > 0){
            if($value == 1){
                ys_admin_logs('显示商户文章分类','cate_art',$id);
            }elseif($value == 0){
                ys_admin_logs('隐藏商户文章分类','cate_art',$id);
            }
            $result = 1;
        }else{
            $result = 0;
    ...
    public function uploadify(){
        ...
        if($file){
            $info = $file->validate(['size'=>3145728,'ext'=>'jpg,png,gif,jpeg'])->move(ROOT_PATH . 'public' . DS . 'uploads' . DS . 'cate_art');
            if($info){
                ...
                $original = 'uploads/cate_art/'.$getSaveName;
        ...
        if(request()->isAjax()){
            ...
            if(true !== $result){
                ...
                if($lastId){
                    ...
                    ys_admin_logs('新增商户文章分类','shop_art',$cate->id);
        ...
        if(request()->isAjax()){
            if(input('post.id')){
                ...
                if(true !== $result){
                    ...
                    $cateinfos = Db::name('cate_art')->where('id',$data['id'])->find();
                    if($cateinfos){
                        if(!empty($data['pic_id'])){
                            $zssjpics = Db::name('huamu_zspic_art')->where('id',$data['pic_id'])->where('admin_id',$admin_id)->find();
                            if($zssjpics && $zssjpics['img_url']){
                        ...
                        if($count !== false){
                            ...
                            ys_admin_logs('编辑商户文章分类','shop_art',$data['id']);
            ...
            if(input('id')){
                $id = input('id');
                $cates = Db::name('cate_art')->where('id',$id)->find();
                if($cates){
                    $admin_id = session('admin_id');
                    $zssjpics = Db::name('huamu_zspic_art')->where('admin_id',$admin_id)->find();
                    if($zssjpics && $zssjpics['img_url']){
                    ...
                    $cateres = Db::name('cate_art')->where('id','neq',$id)->field('id,cate_name,pid')->order('sort asc')->select();
                    $this->assign('cateres', recursive($cateres));
    ...
    public function delete(){
        $id = input('id');
        if(!empty($id)){
            $child = Db::name('cate_art')->where('pid',$id)->field('id')->limit(1)->find();
            if(!empty($child)){
                $value = array('status'=>0,'mess'=>'该分类下存在子分类，删除失败');
            }else{
                $ga = Db::name('shop_news')->where('cate_id',$id)->limit(1)->field('id')->find();
                if(!empty($ga)){
                    $value = array('status'=>0,'mess'=>'该分类存在文章，删除失败');
                }else{
                    $cate_pic = Db::name('cate_art')->where('id',$id)->value('cate_pic');
                    $count = CateNewMX::destroy($id);
                    if($count > 0){
                        if(!empty($cate_pic) && file_exists('./'.$cate_pic)){
                            @unlink('./'.$cate_pic);
                        }
                        ys_admin_logs('删除商户文章分类','cate_art',$id);

...
class IntegralGoods extends Common
{
    public function lst()
    {
        ...
        $list = Db::name('integral_goods')->where('is_recycle', 0)->where($where)->order('id desc')->paginate(25);
        $page = $list->render();
    ...
    public function gaibian()
    {
        ...
        if ($name && $name == 'onsale') {
            ...
            if (isset($value) && in_array($value, array(0, 1))) {
                $goods = Db::name('integral_goods')->where('id', $id)->field('id')->find();
                if ($goods) {
                    $data[$name] = $value;
//                    $data['shop_id'] = $shop_id;
                    ...
                    Db::startTrans();
                    try {
                        Db::name('integral_goods')->update($data);
...
//                            }
                            ...
                            ys_admin_logs('设为上架', 'integral_goods', $id);
...
//                                }
//                            }
                            ys_admin_logs('设为下架', 'integral_goods', $id);
                        }
                        // 提交事务
                        Db::commit();
                        $result = 1;
    ...
    public function uploadifys()
    {
        ...
        $nupload = Db::name('huamu_integralopic')->where('admin_id', $admin_id)->count();
        if (input('post.goods_id')) {
            $goods = Db::name('integral_goods')->where('id', input('post.goods_id'))->find();
            if ($goods) {
                $ycount = Db::name('interal_goods_pic')->where('goods_id', input('post.goods_id'))->count();
...
//                $image->thumb(640, 400)->save('./' . $original, null, 90);
                $original = $info['name'];
                $pic_id = Db::name('huamu_integralopic')->insertGetId(array('img_url' => $original, 'admin_id' => $admin_id));
...
//                    }
    ...
    public function deleteone()
    {
        ...
        if (input('post.ypic_id') && input('post.goods_id')) {
            $goods = Db::name('integral_goods')->where('id', input('post.goods_id'))->field('id')->find();
            if ($goods) {
                $pics = Db::name('interal_goods_pic')->where('id', input('post.ypic_id'))->where('goods_id', input('post.goods_id'))->field('id,img_url')->find();
                if ($pics) {
                    $count = Db::name('interal_goods_pic')->delete(input('post.ypic_id'));
                    if ($count > 0) {
...
//            $data['brand_id'] = 0;
            $data['shop_id'] = $shop_id;
            $result = $this->validate($data, 'IntegralGoods');
            if (true !== $result) {
...
//            $data['search_keywords'] = str_replace('，', ',', $data['search_keywords']);
            ...
            Db::startTrans();
            try {
                $goods_id = Db::name('integral_goods')->insertGetId(array(
                ...
                if ($goods_id) {
                    ...
                    if (!empty($data['picres_id'])) {
                        ...
                        foreach ($data['picres_id'] as $key => $val) {
                            $img_url = Db::name('huamu_integralopic')->where('id', $val)->value('img_url');
                            if ($img_url) {
                                if (empty($sort2[$key])) {
                                    $sort2[$key] = 0;
                                }
                                Db::name('interal_goods_pic')->insert(array('img_url' => $img_url, 'sort' => $sort2[$key], 'goods_id' => $goods_id));
                            }
                        }
                    }
//
...
//                }
                ...
                ys_admin_logs('添加积分商品', 'integral_goods', $goods_id);
        ...
        if (request()->isPost()) {
            if (input('post.id')) {
                ...
                if (true !== $result) {
                    ...
                    $goodss = Db::name('integral_goods')->where('id', input('post.id'))->find();
                    if ($goodss) {
...
//var_dump($data);exit;
                                                ...
                                                Db::startTrans();
                                                try {
                                                    Db::name('integral_goods')->where('id', $data['id'])->update(array(
//                                                        'id' => $data['id'],
                                                        'goods_name' => $data['goods_name'],
                                                        'thumb_url' => $data['thumb_url'],
//                                                        'market_price' => $data['market_price'],
...
//                                                                $sort2[$keypic] = 0;
//                                                            }
//                                                            $goodspics = Db::name('interal_goods_pic')->where('id', $valpic)->where('goods_id', $goods_id)->find();
//                                                            if ($goodspics) {
//                                                                Db::name('interal_goods_pic')->where('id', $valpic)->where('goods_id', $goods_id)->update(array('sort' => $sort2[$keypic]));
//                                                            }
//                                                        }
//                                                    }
                                                    ...
                                                    if (!empty($data['picres_id'])) {
                                                        ...
                                                        foreach ($data['picres_id'] as $key => $val) {
                                                            ...
                                                            if ($img_url) {
                                                                ...
                                                                Db::name('interal_goods_pic')->insert(array('img_url' => $img_url, 'sort' => $sort3[$key], 'goods_id' => $goods_id));
                                                            }
                                                        }
                                                    }
...
//                                                    }
                                                    ...
                                                    ys_admin_logs('编辑积分商品', 'integral_goods', $data['id']);
                                                ...
                                                $value = array('status' => 0, 'mess' => '商品图片最多上传张');
                                            }
                                        } else {
...
//                $shop_id = session('shop_id');
//                $admin_id = session('admin_id');
                $goodss = Db::name('integral_goods')->where('id', input('id'))->find();
                if ($goodss) {
//                    $zssjpics = Db::name('huamu_zspic')->where('admin_id', $admin_id)->find();
//                    if ($zssjpics && $zssjpics['img_url']) {
//                        Db::name('huamu_zspic')->where('id', $zssjpics['id'])->update(array('img_url' => ''));
...
//                    $types = Db::name('type')->where('id', $goodss['type_id'])->field('id,type_name')->find();
                    ...
                    $goodpicres = Db::name('interal_goods_pic')->where('goods_id', input('id'))->order('sort asc')->select();
    ...
    public function product()
    {
        if (request()->isPost()) {
            ...
            if (!empty($data['goods_number']) && !empty($data['goods_id'])) {
                ...
                $goodsinfo = Db::name('integral_goods')->where('id', $goods_id)->where('is_recycle', 0)->find();
                if ($goodsinfo) {
...
//                            if (true) {
            ...
            $id = input('id');
//            $shop_id = session('shop_id');
            $goodsinfo = Db::name('integral_goods')->where('id', input('id'))->where('is_recycle', 0)->find();
            if ($goodsinfo) {
...
//                }
                ...
                $goods_name = Db::name('integral_goods')->where('id', input('id'))->value('goods_name');
...
//        $shop_id = session('shop_id');
        if (!empty($id) && !is_array($id)) {
            $goods = Db::name('integral_goods')->where('id', $id)->find();
            if ($goods) {
                // 启动事务
                Db::startTrans();
                try {
                    Db::name('integral_goods')->where('id', $id)->update(array('is_recycle' => 1, 'onsale' => 0));
//                    $ymanages = Db::name('shop_management')->where('shop_id', $shop_id)->where('cate_id', $goods['cate_id'])->find();
//                    if ($ymanages) {
//                        $good_manages = Db::name('goods')->where('shop_id', $shop_id)->where('cate_id', $goods['cate_id'])->where('onsale', 1)->where('is_recycle', 0)->field('id')->find();
//                        if (!$good_manages) {
...
//                    Db::name('integral_goods')->where('id', $id)->setDec('goods_num', 1);
                    // 提交事务
                    Db::commit();
                    ys_admin_logs('商品加入回收站', 'integral_goods', $id);
    ...
    public function delete()
    {
        ...
        if (!empty($id)) {
            if (!is_array($id)) {
                $goods = Db::name('goods')->where('id', $id)->where('shop_id', $shop_id)->where('is_recycle', 1)->where('onsale', 0)->find();
                if ($goods) {
                    $good_picres = Db::name('interal_goods_pic')->where('goods_id', $id)->field('id,img_url')->select();
                ...
                foreach ($idarr as $vd) {
                    ...
                    if ($goods) {
                        $good_picres = Db::name('interal_goods_pic')->where('goods_id', $vd)->field('id,img_url')->select();
...
//        }
        ...
        $list = Db::name('integral_goods')->alias('a')->field('a.id,a.goods_name,a.thumb_url,a.shop_price,a.onsale')->where($where)->order('a.addtime desc')->paginate(25);
        $page = $list->render();

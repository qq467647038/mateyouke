...
class Order extends Common{
    ...
    public function fictitious_lst(){
        // 土特产商品
        ...
        $list = Db::name('order')->alias('a')
                                    ->field('a.*,b.user_name,b.phone,c.pro_name,d.city_name,u.area_name')
                                    ->join('sp_member b','a.user_id = b.id','LEFT')->join('sp_province c','a.pro_id = c.id','LEFT')
                                    ->join('sp_city d','a.city_id = d.id','LEFT')
                                    ->join('sp_area u','a.area_id = u.id','LEFT')
                                    ->where($where)
                                    ->where('a.fictitious_material', 1)
                                    ->order('a.addtime desc')->paginate(25);
        $page = $list->render();
        ...
        if(request()->isAjax()){
            return $this->fetch('fictitious_ajaxpage');
        }else{
            return $this->fetch('fictitious_lst');
        }
    }

    /**
     * @function待分账订单
     * @desc未收货的订单，筛选出已付款的订单
     * @author Feifan.Chen <1057286925@qq.com>
     * @return mixed
     */
    public function waitSeparateOrder(){
        $shop_id = session('shop_id');
        $where = ['a.shop_id'=>$shop_id,'a.state'=>1,'a.order_status'=>0];
        ...
        $list = Db::name('order')
            ...
            ->join('sp_province c','a.pro_id = c.id','LEFT')
            ->join('sp_city d','a.city_id = d.id','LEFT')
            ->join('sp_area u','a.area_id = u.id','LEFT')
            ->where($where)
            ->whereIn('a.fh_status',[0,1])
        ...
        $prores = Db::name('province')
            ->field('id,pro_name,zm')->order('sort asc')->select();
        $this->assign('prores',$prores);
        $this->assign('pnum',$pnum);
        $this->assign('page',$page);// 赋值分页输出
        $this->assign('list',$list);// 赋值数据集
        if(request()->isAjax()){
            return $this->fetch('sp_ajaxpage');
        }else{
            return $this->fetch('sp_lst');
        }
    }
    ...
    public function getproof()
    {
        ...
        if(request()->isPost()){
            ...
            $res = Db::name('order')->where('id', $input['id'])->update([
                'state' => 1,
                'zf_type' => 10,
            ]);
            ...
            if($res)
            {
                return $this->success('确认已支付成功');
            }

            return $this->error('确认已支付失败');
        }
    ...
    public function setUse(){
        if (request()->isPost()){
            $is_use = $this->request->post('is_use',0);
            $order_id = $this->request->post('order_id');
            $update = Db::name('order')->where('id',$order_id)->update(['is_use'=>$is_use]);
            if ($update){
    ...
    public function delete(){
        if(input('id') && !is_array(input('id'))){
            ...
            $orders = Db::name('order')->where('id',$id)->where('shop_id',$shop_id)->where(function ($query){
                $query->where('state', '<>', 1)->whereOr('order_status', '<>', 1)->whereOr('fh_status', '<>', 1);
            })->field('id')->find();
            if($orders){
                // 启动事务
                Db::startTrans();
    ...
    public function search(){
        ...
        if(input('keyword') != ''){
            cookie('or_keyword',input('keyword'),7200);
        }else{
        ...
        if(cookie('or_keyword')){
            $where['a.ordernumber|b.phone'] = cookie('or_keyword');
        }
        ...
        $isAjax = 'ajaxpage';
        $fetch = 'lst';
        if(input('param.goods_type') || input('param.goods_type')=='travel')
        {
            $map['a.fictitious_material'] = 1;
            $isAjax = 'fictitious_ajaxpage';
            $fetch = 'fictitious_lst';
        }

        $six_month_time = strtotime('-6 month');
        if(input('param.time_limit') == 1)
        {
            // 大于6个月的订单
            $where['a.addtime'] = ['elt', $six_month_time];
        }

        if(input('param.time_limit') == 2)
        {
            // 6个月内的订单
            $where['a.addtime'] = ['egt', $six_month_time];
        }
        $this->assign('time_limit',input('param.time_limit'));

        $list = Db::name('order')->alias('a')->field('a.*,b.user_name,b.phone,c.pro_name,d.city_name,u.area_name')->join('sp_member b','a.user_id = b.id','LEFT')->join('sp_province c','a.pro_id = c.id','LEFT')->join('sp_city d','a.city_id = d.id','LEFT')->join('sp_area u','a.area_id = u.id','LEFT')->where($where)->where($map)->order('a.addtime desc')->paginate(25,false,['query'=>request()->param()]);
    ...
    public function export(){
        ...
        $list = Db::name('order')->alias('a')->field('a.*,b.user_name,b.phone,c.pro_name,d.city_name,u.area_name')->join('sp_member b','a.user_id = b.id','LEFT')->join('sp_province c','a.pro_id = c.id','LEFT')->join('sp_city d','a.city_id = d.id','LEFT')->join('sp_area u','a.area_id = u.id','LEFT')->where($where)->where($map)->order('a.addtime desc')->select();
        
     
        $arr = [];
        array_push($arr, ['A'=>'订单号', 'B'=>'类型', 'C'=>'订单状态', 'D'=>'会员昵称', 'E'=>'会员手机', 'F'=>'总价', 'G'=>'所属区域', 'H'=>'下单时间']);
        $order_type_arr[0] = "普通订单";
        $order_type_arr[1] = "解锁订单";
        $order_type_arr[2] = "激活订单";
        $order_type_arr[3] = "积分订单";
        $order_type_arr[4] = "";
     
        foreach($list as $k=>$v){
            $v['agent_type'] = max($v['agent_type'],$v['false_agent_type']);
            $state = "";
            if($v['state'] == 1 && $v['fh_status'] == 0 && $v['order_status'] == 0){
                $state = "待发货";
            }else if($v['state'] == 1 && $v['fh_status'] == 1 && $v['order_status'] == 0){
                $state = "已发货";
            }
            else if($v['state'] == 1 && $v['fh_status'] == 1 && $v['order_status'] == 1){
                $state = "已完成";
            }
            else if($v['state'] == 0 && $v['fh_status'] == 0 && $v['order_status'] == 0){
                $state = "待支付";
            }
            ...
            array_push($arr, ['A'=>$v['ordernumber'], 'B'=>$order_type_arr[$v['type']], 'C'=>$state, 'D'=>$v['user_name'], 'E'=>$v['phone'], 'F'=>$v['total_price'], 'G'=>$v['pro_name'] . "-".$v['city_name']."-".$v['area_name'], 'H'=>date("Y-m-d H:i:s",$v['addtime'])]);
        }

//栏目分类列表，友情链接列表，权限列表，角色列表公用js
	...
	//修改分类显示、导航显示、首页推荐位显示
	function changeCommunityFeedVal(field_id,field_name,obj){
	...
	//修改分类显示、导航显示、首页推荐位显示
	function changeCommunityArticleVal(field_id,field_name,obj){
		if($(obj).hasClass('btn-danger')){
			var field_value = 1;
		}else if($(obj).hasClass('btn-primary')){
			var field_value = 0;
	...
	//修改分类显示、导航显示、首页推荐位显示
	function changeFeedTableVal(field_id,field_name,obj){
		if($(obj).hasClass('btn-danger')){
			var field_value = 1;
		}else if($(obj).hasClass('btn-primary')){
			var field_value = 0;
	...
	//修改分类显示、导航显示、首页推荐位显示
	function changeVideoTableVal(field_id,field_name,obj){
		if($(obj).hasClass('btn-danger')){
			var field_value = 1;
		}else if($(obj).hasClass('btn-primary')){
			var field_value = 0;
	...
	//修改分类显示、导航显示、首页推荐位显示
	function changeCoursTableVal(field_id,field_name,obj){
		if($(obj).hasClass('btn-danger')){
			var field_value = 1;
		}else if($(obj).hasClass('btn-primary')){
			var field_value = 0;
	...
	function delvideoone(id,obj){
		layer.confirm('确定要删除么?',{
			...
			post(deleteVideoUrl,'GET',{'id':id},1);
			return false;
		});
	}

	//单个删除
	function delcourseone(id,obj){
		layer.confirm('确定要删除么?',{
			skin: 'layui-layer-molv',
			closeBtn: 0,
			shadeClose : true,
			...
			post(deleteCourseUrl,'GET',{'id':id},1);
			return false;
		});
	}
